...
- **User / settings (dev)** (below the main row, inside the drawer):
  ...
  - Tonight and remaining-time controls use the same preference setters and `syncUserSettingsRowToCloud()` as Settings when cloud sleep data is active. **Tonight guidance** mirrors `getTonightGuidanceEnabled` / `setTonightGuidanceEnabled` and `getTonightGuidancePaceId` / `setTonightGuidancePaceId` (`sleep-app-tonight-guidance-enabled`, `sleep-app-tonight-guidance-pace`); updates sync to `user_settings`, dispatch `tonight-guidance-changed`, and refresh the dev-banner panel.
...
| `TONIGHT_GUIDANCE_ENABLED_KEY` | `sleep-app-tonight-guidance-enabled` | `'1'` when Tonight guidance is on (local mirror of `user_settings.tonight_guidance_enabled`) |
