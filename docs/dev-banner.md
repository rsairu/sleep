...
- **Badges row** (`nav-dev-banner-badges-row`): one flex line in order **GitHub**, **Vercel + deployment**, **app time**. **GitHub** (`nav-dev-banner-github-badge`): GitHub mark + **github**; value = git-branch icon + branch from `window.__DEV_GIT_BRANCH__` (**passing** green when branch set and not `master`, **failing** red on `master`, **unknown** gray otherwise). **Vercel** (`nav-dev-banner-vercel-badge`): single flat badge; whole control links to **`DEV_VERCEL_PROJECT_URL`**. Left: Vercel triangle SVG + **vercel**. Right: `#nav-dev-banner-vercel-deploy-status` shows **pass** or **fail** (display via `formatVercelDeployDisplayLabel()` from Shields JSON tone; Shields `message` e.g. success maps to **pass**). Data from live [Shields JSON](https://img.shields.io/github/deployments/rsairu/sleep/Production.json). When the resolved tone is **passing**, `writeVercelDeployShieldsPassCache()` stores an expiry (`VERCEL_DEPLOY_SHIELDS_PASS_CACHE_MS`, 30 minutes); while valid, `readVercelDeployShieldsPassCache()` skips the network fetch and keeps showing **pass**. Non-passing or fetch errors clear the cache (`clearVercelDeployShieldsPassCache()`). **App time** (`nav-dev-banner-apptime-badge`, not a link): clock SVG + **app time**; value **real** / **sim** as before. `#nav-dev-banner-apptime-badge-value` carries `role="status"` and `aria-live="polite"`.
- **Cloud row** (`nav-dev-banner-cloud-row`): one flex line in order **Supabase** (`nav-dev-banner-supabase-badge`), **Switch DB** (`#nav-dev-banner-db-switch-toggle`, emoji-only control with full `aria-label` / title), **Sync with cloud** (`#nav-dev-banner-cloud-refresh-btn`, emoji-only). **Supabase**: mark + **supabase**; value **dev** / **prod** / **n/a** / **unknown** with the same green/red/gray rules as before; links to the Supabase dashboard. **Switch DB** always renders. When `local-supabase-presets.js` is valid it flips dev ↔ prod; when presets are missing/invalid it is disabled (grayed) and shows tooltip text **"Please see local-supabase-presets.example.js"**. **Sync with cloud** runs `loadSleepData({ forceRefresh: true })` then reloads; disabled when Supabase is not configured.
...
- **App time (right column)**: `#nav-dev-banner-clock` holds the time panel (`#nav-dev-banner-clock-sim-panel`): `datetime-local` (`#nav-dev-banner-dev-clock-input`), then one toolbar row (`nav-dev-banner-clock-step-toolbar`, same width as the input row via flex-filling buttons) with **nine** controls in order: previous day, minus one hour (triple chevron), minus five minutes (double chevron), minus one minute, **clock reset** (`#nav-dev-banner-clock-reset-real-btn`, slightly wider than step buttons, clock SVG), plus one minute, plus five minutes, plus one hour (triple chevron), next day; reset clears `sleep-app-dev-clock-override-ms` and reloads when an override exists. Editing the datetime or using steps persists override (reload) so the badge value becomes **sim**.
...
- badges row (GitHub / Vercel JSON deployment status / app time), cloud row (Supabase shield + Switch DB + Sync), or right-column clock panel IDs and copy,
